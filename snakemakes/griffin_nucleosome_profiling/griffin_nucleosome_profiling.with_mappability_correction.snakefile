...
snakemake -s griffin_nucleosome_profiling.with_mappability_correction.snakefile --cores 1 -np

#command to run snakemake on a slurm cluster (remove -np at end when done validating):
snakemake -s griffin_nucleosome_profiling.with_mappability_correction.snakefile --latency-wait 60 --keep-going --cluster-config config/cluster_slurm.yaml --cluster "sbatch -p {cluster.partition} --mem={cluster.mem} -t {cluster.time} -c {cluster.ncpus} -n {cluster.ntasks} -o {cluster.output} -J {cluster.JobName}" -j 40 -np
