...
# max_fetch_length = 50000
# if to_fetch['length'].max()>max_fetch_length:
#     short_intervals = to_fetch[to_fetch['length']<=max_fetch_length]
#     long_intervals = to_fetch[to_fetch['length']>max_fetch_length]
#     print('Splitting',len(long_intervals),'intervals that exceed the maximum fetch length of',max_fetch_length,'bp')

#     split_intervals=pd.DataFrame()
#     for i in range(len(long_intervals)):
#         current = long_intervals.iloc[i]
#         starts = np.arange(current['start'],current['end'],max_fetch_length)
#         chrom = current['chrom']
#         max_end = current['end']
#         for start in starts:
#             end = start+max_fetch_length
#             if end>max_end:
#                 end=max_end
#             split_intervals = split_intervals.append({'chrom':chrom,'start':start,'end':end},ignore_index=True)
        
#         if (i+1)%5000==0:
#             printout = griffin_functions.progress_report([chrom,start,end],'intervals',start_time,time.time(),i,len(long_intervals))
#             print(printout)
            
#     split_intervals['start'] = split_intervals['start'].astype(int)
#     split_intervals['end'] = split_intervals['end'].astype(int)
#     split_intervals['length'] = split_intervals['end'] - split_intervals['start']


#     to_fetch = short_intervals.append(split_intervals, ignore_index=True)
#     del(long_intervals,chrom,split_intervals,short_intervals)
